...
- Live restore test (Manny, 14 Sep): confirm staged the file and the pre-restore snapshot, then the process hung in uvicorn's graceful shutdown for over 90 seconds with `/healthz` not answering and Docker still reporting healthy from a stale check. Fix, no gate: shutdown must never depend on graceful completion. uvicorn now runs with a 5 second graceful timeout; every lifespan teardown step is bounded (proxy sessions abandoned after 2 seconds, other steps 5); a watchdog thread forces exit 15 seconds after shutdown begins; `/healthz` returns 503 once shutdown has begun and the container healthcheck polls every 10 seconds with two retries. The restart button and restore confirm go through the same controller, and uvicorn's signal handler is hooked to it so a Docker stop behaves the same. Test: a restart with a live proxy session whose transport never finishes closing completes teardown within the bound.
- Found while writing that test: sse_starlette, pulled in by the mcp SDK, replaces uvicorn's `Server.handle_exit` at import and sets a process-wide flag that ends every SSE response from then on. Correct in production, and the reason our own `handle_exit` override must call through to the base class. In tests the flag has to be reset by hand or every later streamable HTTP response dies without a body.
